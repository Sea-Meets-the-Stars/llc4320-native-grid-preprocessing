...
How much better, in numbers? The estimate below combines analytic error propagation with a synthetic numerical experiment (512² C-grid-sampled flow with a k⁻² velocity spectrum, spectral truth; both pipelines' exact stencils emulated). Setup facts, verified in code: the old pipeline extracted cutouts on the LLC4320 lat-lon sector, whose true spacing is ≈ 2.31·cos(lat) km (1/48°, Mercator-like isotropy), computed every derived field *before* the 64-px resize with `np.gradient`, and divided by a **constant 2.25 km** regardless of latitude.

**A. Metric bias (all old gradient-based fields).** Because the assumed spacing is right only near the equator, every old gradient amplitude carries a multiplicative bias of `2.31·cos(lat)/2.25`, squared for the `|∇·|²` family:

| lat | true spacing | 1st-derivative fields (vorticity, divergence, strain) | squared-gradient fields (Divb2, DivSST2, OW) |
|---|---|---|---|
| 0° | 2.31 km | +3% | +5% |
| 30° | 2.00 km | −11% | −21% |
| 45° | 1.63 km | −27% | −47% |
...
Area-weighted over the old sampled domain (−70° to +57°): **−14% mean bias for first derivatives, −23% for squared gradients**, rising past −70% in the Southern Ocean — exactly where LLC4320's frontal activity is strongest. Crucially this error is *systematic and latitude-correlated*, not noise: a model trained to predict old-Divb2 amplitude learns a spurious cos²(lat) suppression of tens of percent baked into its targets.
