...
def _make_synthetic_grid_face(n_k: int) -> xr.Dataset:
    ...
    ds = xr.Dataset(
        ...
        coords={
            "face": np.array([0]),
            "k":    np.arange(n_k),
            # The real store carries these (stamped by
            # get_llc_depth_gridfile); xgcm finds X/Y through them.
            **{name: xr.DataArray(
                   np.arange(n_i if name.startswith("i") else n_j),
                   dims=name, attrs=meta)
               for name, meta in COMODO_COORD_META.items()},
        },
    )
    return ds
