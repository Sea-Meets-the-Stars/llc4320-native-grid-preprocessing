...
def _field(j=5, i=6):
    """Increasing (j, i) field — mirrors a materialized ds_merge feature."""
    return np.arange(1, j * i + 1, dtype="float32").reshape(j, i)
...
def test_returns_positional_ji_indices_respecting_mask():
    ...
    idx = weighted_sample_on_grid(5, 1.3, arr, mask)
...
def test_recovers_exact_cells_when_all_but_k_masked():
    ...
    for j, i in keep:
        mask[j, i] = True
    idx = weighted_sample_on_grid(3, 1.3, arr, mask)
    assert set(idx) == keep
