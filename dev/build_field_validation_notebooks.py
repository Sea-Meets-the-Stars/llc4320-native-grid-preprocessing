...
# Wstar, frontogenesis_tendency, |grad b| over the Kerguelen Plateau.
...
# |grad b| from the live gradient components (not a store channel).
_xk, _yk, _dbx = _kerg("db_dx")
_dby = _kerg("db_dy")[2]
gradb_mag_kerg = np.hypot(_dbx, _dby)


def _kerg_panel(arr, cmap_field):
    \"\"\"Renderer for one Kerguelen map panel.

    Inputs: arr (2D np.ndarray); cmap_field (str) — field_cmaps key.
...
stacked_side_by_side(
    [_kerg_panel(_kerg("Wstar")[2], "Wstar"),
     _kerg_panel(_kerg("frontogenesis_tendency")[2],
                 "frontogenesis_tendency"),
     _kerg_panel(gradb_mag_kerg, "gradb_mag")],
    LIT_DIR / ("Wstar-F-gradb_Bachman-etal(2021)_"
               "Kerguelen-maps.png"),
    our_titles=["Wstar", "frontogenesis_tendency", "|\u2207b|"],
    caption=("Bachman et al. (2021), Kerguelen Plateau.  Expect the "
             "same morphology: W* dipoles around eddies, filamentary "
             "positive F along strain-dominated fronts collocated "
             "with |\u2207b| ridges.  Different model + date: "
             "compare structure/magnitude, not features."),
)
plt.show()\
"""

BACHMAN_PDFS_MD = """\
### 6.2 Bachman et al. (2021) — log-magnitude PDFs, same region
...
def _bachman_pdfs(ax):
    ...
    panels = [
        ("log\u2081\u2080 |W*|", np.abs(_kerg("Wstar")[2]),
         "indianred"),
        ("log\u2081\u2080 |F|",
         np.abs(_kerg("frontogenesis_tendency")[2]), "steelblue"),
...
side_by_side(
    ...
    caption=("Compare modal positions and widths with Bachman et "
             "al. (2021) Fig 2: |F| ~1e-16, |W*| and |\u2207b| "
             "~1e-8\u20131e-7, |\u2207b| slightly right of |W*|."),
)
plt.show()\
"""
...
| FIELD NAME | UNITS | EQUATION | DEPENDENCIES | LOCATION OF CALC IN CODE |
|---|---|---|---|---|
| frontogenesis_tendency | s⁻² | F(u,v) from J and ∇b | J (U, V); db_dx, db_dy (← b ← Theta, Salt) | `calculate_fields.frontogenesis_tendency` (`_frontogenesis_formula`) |
...
| frontogenesis_geo | s⁻² | F(ug,vg) | ug, vg gradients; db_dx, db_dy | `calculate_fields.frontogenesis_geo` |
| frontogenesis_ageo | s⁻² | F(u,v) − F(ug,vg) | both tendencies | `surface_subsets.compute_frontogenesis` (inline) |
| Wstar | s⁻² | modified Okubo-Weiss W* = 4·sgn(l₂)·√(l₁²+l₂²) (Bachman 2021) | J; db_dx, db_dy; f | `calculate_fields.modified_okubo_weiss` |
...
""",
    ...
    log_fields=set(),
    fields=[
        ("frontogenesis_tendency", "5.1 frontogenesis_tendency",
         "F(u,v) from J and ∇b: positive filaments along strain-"
         "dominated fronts (Gulf Stream north wall, ACC)."),
        ...
        ("frontogenesis_geo", "5.4 frontogenesis_geo",
         "F(ug,vg): geostrophic contribution; smoother than the "
         "full tendency."),
        ("frontogenesis_ageo", "5.5 frontogenesis_ageo",
         "F(u,v) − F(ug,vg): ageostrophic/submesoscale part "
         "(dispatcher-inline difference)."),
        ("Wstar", "5.6 Wstar (modified Okubo-Weiss)",
         "Bachman (2021) W* using ∇b and f; NaN-heavy at the "
         "equator (expected; PDF row filtered)."),
    ],
