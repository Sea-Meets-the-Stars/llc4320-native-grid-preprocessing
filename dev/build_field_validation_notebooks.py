...
for i0 in range(0, len(names), BATCH):
    ...
    for k, n in enumerate(grp):
        region_arrays[n] = regions.select_all_regions(chw[k], XC, YC)
    del chw
...
""",
    chains={
        "frontogenesis_tendency": ["U", "V", "db_dx", "db_dy",
...
def build(subset, spec):
    ...
    for entry in spec["fields"]:
        ...
        fig3 = entry[3] if len(entry) > 3 else (
            f'figure3_literature("{field}")')
        md(cells, f"### {title}\n\n{desc}")
