...
def buoyancy_of_field(ds_merge):
    """Buoyancy b = G * rho / RHO0 from potential density (lazy).

    The buoyancy definition: G = 9.81 m s^-2 and
    RHO0 = RHO0_BOUSSINESQ = 1000 kg m^-3, consistent with the
    N2 = (G / RHO0) drho/dz used for stratification, so dimensionless
    diagnostics built from N2 and horizontal buoyancy gradients (R_ib,
    W*) are internally consistent.

    Not an anomaly (no reference subtraction, no negative sign): every
    consumer differentiates b, so constant offsets vanish.
    ...
    Generated by LH and Claude
    """
    b = G * potential_density(ds_merge) / RHO0_BOUSSINESQ
    b.name = "buoyancy"
    b.attrs["long_name"] = "buoyancy b = g rho / rho0 (JMD95 sigma-theta)"
    b.attrs["units"] = "m s-2"
    return b
