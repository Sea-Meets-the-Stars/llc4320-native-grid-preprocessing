...
def kinetic_energy(ds_merge, grid, mld=None):
    """Mixed-layer eddy kinetic energy scale KE = 0.5 (MLD |grad b| / f)^2.

    Promoted from ``depth_subsets.compute_energetics`` (field-validation
    Clarification 4) so the formula has one canonical, citable location.
    Dimension-agnostic and lazy, like the other functions here.

    Parameters
    ----------
    ds_merge : xr.Dataset
        Merged dataset with ``Theta``, ``Salt``, latitude ``YC``, grid
        metrics ('dxC', 'dyC') and rotation coefficients ('CS', 'SN').
    grid : xgcm.Grid
        Grid object used for differencing and interpolation.
    mld : xr.DataArray, optional
        Pre-computed mixed-layer depth [m].  Computed via
        :func:`mixed_layer_depth` when *None*.

    Returns
    -------
    xr.DataArray
        KE [m^2 s^-2] on tracer points, dask-backed, broadcast over the
        vertical dimension of ``grad b``.
