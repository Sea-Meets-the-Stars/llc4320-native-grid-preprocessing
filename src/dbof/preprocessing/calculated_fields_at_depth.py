...
def _density_lazy(ds_merge):
    """Thin wrapper over :func:`calculate_fields.potential_density`.
...
def buoyancy_field_3d(ds_merge):
    """Thin wrapper over :func:`calculate_fields.buoyancy_of_field`.

    Single project-wide buoyancy definition b = G rho / RHO0_REFERENCE
...
def _buoyancy_gradient_phys_3d(ds_merge, grid):
    ...
    *physical* buoyancy b = (g / ρ₀) ρ — the same unscaled definition
    ...
    ``RHO0_REFERENCE``).  This is the single source of truth for the
...
def _buoyancy_gradient_squared_phys_3d(ds_merge, grid):
    ...
    density ``RHO0_REFERENCE``, no extra scaling).  Deriving the gradient
