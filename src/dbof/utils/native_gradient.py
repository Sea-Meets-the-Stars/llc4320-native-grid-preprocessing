...
from dbof.llc4320_ingestion.grid import face_seam_mask
...
def interp_pair_to_center(x_component, y_component, grid):
    ...
    """
    # successor to xgcm's deprecated interp_2d_vector
    return (grid.interp({'X': x_component}, 'X', padding='fill',
                        other_component={'Y': y_component}),
            grid.interp({'Y': y_component}, 'Y', padding='fill',
                        other_component={'X': x_component}))
...
def diff_pair_along_own_axis(x_component, y_component, grid):
    ...
    Generated by LH and Claude
    """
    return (grid.diff({'X': x_component}, 'X', padding='fill',
                      other_component={'Y': y_component}),
            grid.diff({'Y': y_component}, 'Y', padding='fill',
                      other_component={'X': x_component}))
...
def diff_pair_across_other_axis(x_component, y_component, grid):
    ...
    ----------
    x_component, y_component : xarray.DataArray
        Staggered pair, already multiplied by their metrics.
    grid : xgcm.Grid

    Returns
    -------
    (xarray.DataArray, xarray.DataArray)
        ``x`` differenced along Y and ``y`` differenced along X, both on
        cell corners.

    Generated by LH and Claude
    """
    return (grid.diff({'X': x_component}, 'Y',
                      other_component={'Y': y_component}),
            grid.diff({'Y': y_component}, 'X',
                      other_component={'X': x_component}))
...
def calculate_grad_squared_tracer(ds_value, ds_grid, grid):
    ...
    dx2_c = grid.interp(ds_dx_M ** 2, 'X', padding='fill')
    dy2_c = grid.interp(ds_dy_M ** 2, 'Y', padding='fill')
...
def calculate_grad_dot_tracer(da_a, da_b, ds_grid, grid):
    ...
    return (grid.interp(ax * bx, 'X', padding='fill')
            + grid.interp(ay * by, 'Y', padding='fill'))
...
def calculate_native_strain_vorticity(u_x, v_y, ds_grid, grid):
    ...
    vorticity_corner = face_seam_mask(vorticity_corner, grid)
    strain_shear_corner = face_seam_mask(strain_shear_corner, grid)
...
def interp_corner_squared(q_corner, grid):
    ...
    Generated by LH and Claude
    """
    return grid.interp(grid.interp(q_corner, 'X', padding='fill'),
                       'Y', padding='fill')
